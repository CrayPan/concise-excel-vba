...
代码详解
1、`d.CompareMode = vbTextCompare`  ：设置字典的比较模式是文本，在这种比较模式下不区分关键字的大小写，即关键字”b”和`”B”`是一样的。`vbTextCompare`的值为1，所以上式也可写为 `d.CompareMode =1` 。如果设置为`vbBinaryCompare`（值为0），则执行二进制比较，即区分关键字的大小写，此种情况下关键字”b”和”B”被认为是不一样的。
